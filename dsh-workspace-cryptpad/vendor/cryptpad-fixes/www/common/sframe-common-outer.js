...
define([
    ...
    common.initIframe = function (waitFor, isRt, pathname) {
        if (window.top !== window) {
            // NOTE(dsh-workspace-cryptpad sidecar, modified 2026-08-30): the whole CryptPad UI is framed by
            // the DSH desktop shell. Allow embedding on any page — the server-side
            // CSP (frame-ancestors) already gates framing via the ENABLE_EMBEDDING
            // decree, so the per-app allow-list below is lifted.
        ...
        nThen(function (waitFor) {
            ...
            var addCommonRpc = function (sframeChan, safe) {
                ...
                var openURL = function (url) {
                    ...
                    if (!a) {
                        // [dsh-workspace-cryptpad, modified 2026-08-30] 内嵌 iframe 里 window.open 非用户手势会被浏览器拦截
                        // （即使 iframe 带 allow="popups"，Permission Policy 只解除策略、不豁免手势检查），
                        // 返回 null → CryptPad 报「需要允许弹窗」。回退为当前 iframe 内导航，工作台内可用。
                        window.location.href = url;
                    }
                };
            ...
            sframeChan.on('EV_OPEN_VIEW_URL', function () {
                ...
                var a = window.open(url);
                if (!a) {
                    // [dsh-workspace-cryptpad, modified 2026-08-30] 同 openURL：弹窗被拦 → 当前 iframe 内导航兜底
                    window.location.href = url;
                }
            });
